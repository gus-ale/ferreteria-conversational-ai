...
INJECTION_PATTERNS = [
    re.compile(r"\bignora(?:r)?\b.{0,50}\binstrucciones?\b", re.IGNORECASE),
    re.compile(r"\bignore\b.{0,50}\b(previous|system|developer)\b", re.IGNORECASE),
...
DESTRUCTIVE_PATTERNS = [
    ...
    re.compile(
        r"\b(borra|borrá|elimina|eliminá)\b.{0,40}\b(todo|tabla|base)\b",
        re.IGNORECASE,
    ),
]
...
def inspect_input(text: str, *, max_characters: int) -> GuardrailDecision:
    if len(text) > max_characters:
        return GuardrailDecision(
            ...
            reason=f"El mensaje supera el límite de {max_characters} caracteres.",
        )
    ...
    for category, patterns in checks:
        if any(pattern.search(text) for pattern in patterns):
            return GuardrailDecision(
                allowed=False,
                category=category,
                reason="La solicitud contradice los límites de seguridad del asistente.",
            )
...
def sanitize_output(text: str) -> str:
    ...
    for pattern in OUTPUT_SECRET_PATTERNS:
        sanitized = pattern.sub("[REDACTADO]", sanitized)
    return sanitized
